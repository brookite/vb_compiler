...
struct expr_node : public node {
	...
	expr_node* left; // для MyClassMemberAccess, MyBaseMemberAccess не заполняется
	expr_node* right; 
...
// В каждом классе нужно реализовать метод dot из общего предка - класса node. Эти реализации пишутся в nodes.cpp файле